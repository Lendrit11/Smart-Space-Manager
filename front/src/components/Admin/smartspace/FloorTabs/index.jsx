  ...
  export default function FloorTabs({ floors, selected, onSelect, onAdd, onRename, onDelete }) {
    ...
    return (
      <div className="flex items-center gap-3">
       <nav className="flex items-center gap-2 p-1 rounded-full" style={{backgroundColor:'transparent'}}>
          {floors.map((f) => (
            <div key={f.id} className="relative">
              {editingId === f.id ? (
                <div className="flex items-center gap-1">
                  ...
                  <button
                    onClick={saveEdit}
                    className="ml-1 px-2 py-1 rounded-md bg-indigo-500 hover:bg-indigo-600 text-sm"
                  >
                    Save
                  </button>
                  <button
                    onClick={() => setEditingId(null)}
                    className="ml-1 px-2 py-1 rounded-md bg-slate-600 hover:bg-slate-500 text-sm"
                  >
                    Cancel
                  </button>
                </div>
                ...
                >
                  ...
                  <span
                    ...
                    className="ml-1 text-xs text-slate-400 hover:text-slate-100 cursor-pointer"
                    aria-label={`Edit ${f.name}`}
                  >
                    ✏️
                  </span>

                  {/* BUTONI DELETE - SHFAQET VETËM KUR KA MË SHUMË SE 1 KAT */}
                  {floors.length > 1 && (
                    <span
  ...
  FloorTabs.propTypes = {
    ...
    onDelete: PropTypes.func.isRequired, // SHTUAR PROP I RI
  };